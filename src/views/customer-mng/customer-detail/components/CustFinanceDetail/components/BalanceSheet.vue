      ...
      getBalanceSheet({
        ...
        .then(res => {
          if (res.data && Array.isArray(res.data.records) && res.data.records.length) {
            ...
            // 流动资产、非流动资产、流动负债、非流动负债、所有者权益、资产总计
            const itemArr = ['BSA10011101', 'BSA10021701', 'BSA10031201', 'BSA10040701', 'BSA10050501', 'BSA10021702']
            ...
            itemArr.forEach(item => {
              tempArry.map(x => {
                if (item == x.accountCode) {
                  finalTemp.push(x)
                  // 流动资产、非流动资产
                  if (['BSA10011101', 'BSA10021701'].includes(x.accountCode)) {
                    this.capitalPie.push(x.closingBalance)
                  }
                  // 流动负债、非流动负债
                  if (['BSA10031201', 'BSA10040701'].includes(x.accountCode)) {
            ...
            this.balanceSheetData = {
              xAxis: {
                data: ['流动资产', '非流动资产', '流动负债', '非流动负债', '所有者权益', '资产总计']
              },
              series: [
                {
                  name: '期初',
