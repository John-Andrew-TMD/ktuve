...
DocsAPI.DocEditor = function(placeholderId, config) {
  ...
  var _callLocalStorage = function(data) {
    if (data.cmd == 'get') {
      if (data.keys && data.keys.length) {
        var af = data.keys.split(','); var re = af[0]
  ...
  var _queryReplaceAll = function(data) {
    _sendCommand({
      command: 'queryReplaceAll',
      data: data
    })
  }

  var _setTrackRevisions = function(data) {
    _sendCommand({
      command: 'setTrackRevisions',
      data: data
    })
  }

  var _addNewTextLine = function(data) {
    _sendCommand({
      command: 'addNewTextLine',
      data: data
    })
  }

  var _setContentControlTexts = function(data) {
    _sendCommand({
      command: 'setContentControlTexts',
      data: data
    })
  }

  var _setContentControlText = function(data) {
    _sendCommand({
      command: 'setContentControlText',
      data: data
    })
  }

  var _removeAllContentControls = function(data) {
    _sendCommand({
      command: 'removeAllContentControls',
      data: data
    })
  }

  var _deleteContentControl = function(tag) {
    _sendCommand({
      command: 'deleteContentControl',
      data: tag
    })
  }
  ...
  var _addRowBelow = function(data) {
    _sendCommand({
      command: 'addRowBelow',
      data: data
    })
  ...
  var _removeTableRow = function(data) {
    _sendCommand({
  ...
  var _setCellText = function(data) {
    _sendCommand({
      command: 'setCellText',
      data: data
    })
  ...
  var _setCellsText = function(data) {
    _sendCommand({
      command: 'setCellsText',
      data: data
    })
  }

  var _addImageToTableCell = function(data) {
    _sendCommand({
      command: 'addImageToTableCell',
      data: data
    })
  }
  ...
  var _clearParagraphBackground = function(data) {
    _sendCommand({
      command: 'clearParagraphBackground',
      data: data
    })
  }

  var _getFormData = function(tag) {
    _sendCommand({
      command: 'getFormData',
      data: tag
    })
  ...
  return {
    ...
    queryReplaceAll: _queryReplaceAll,
    setTrackRevisions: _setTrackRevisions,
    setContentControlText: _setContentControlText,
    setContentControlTexts: _setContentControlTexts,
    removeAllContentControls: _removeAllContentControls,
    deleteContentControl: _deleteContentControl,
    addNewTextLine: _addNewTextLine,
    setMailMergeRecipients: _setMailMergeRecipients,
    mergeCells: _mergeCells,
    addRowBelow: _addRowBelow,
    removeTableRow: _removeTableRow,
    setCellText: _setCellText,
    setCellsText: _setCellsText,
    addImageToTableCell: _addImageToTableCell,
    setWatermark: _setWatermark,
    clearParagraphBackground: _clearParagraphBackground,
...
function MessageDispatcher(fn, scope) {
  var _fn = fn
...
function getBasePath(config) {
  ...
  //     }
  // }
  // const callbackUrl = config.editorConfig && config.editorConfig.callbackUrl || ''
  // const matchs = callbackUrl.match(/^http(s)?:\/\/(.*?)\//)
  // if(matchs && matchs[1]) {
  //   const url = matchs[1].split(':')[0]
  // }
  return `${config.server}/web-apps/apps/`
}
...
function getAppPath(config) {
  ...
  var path = extensionPath || getBasePath(config)
  var appMap = {
    'text': 'documenteditor',
...
function getAppParameters(config) {
  ...
  if (config.editorConfig && config.editorConfig.targetApp !== 'desktop') {
    ...
    } else { params += '&customer=KTFCPOFFICE' }
