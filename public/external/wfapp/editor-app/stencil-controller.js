...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', '$location', function ($rootScope, $scope, $http, $modal, $timeout, $location) {
        ...
        $scope.formModalConfig = {
            showFormConfigModal: false,
            showFormConfigItem: false
        };
        ...
        $scope.propertyWindowState.toggle = function () {
            ...
            var value = $scope.propertyWindowState.collapsed;
            var propertyCollaspedValue = '1%', propertyExtendValue = '20%',
                canvasCollapsedValue = '82%', canvasExtendValue = '63%';
            //当前编辑类型为流程方案
            if($scope.editType == 'solution') {
                propertyCollaspedValue = '1%';
                propertyExtendValue = '25%';
                canvasCollapsedValue = '82%';
                canvasExtendValue = '58%';
            }

            jQuery('#propertiesHelpWrapper').css({width: value ? propertyCollaspedValue : propertyExtendValue})
            jQuery('#canvasHelpWrapper').css({width: value ? canvasCollapsedValue : canvasExtendValue})
            $timeout(function () {
                jQuery(window).trigger('resize');
            });
        };

        $scope.clickGroupType = function(index) {
            $scope.propertyGroup.some(function(item) {
                if(item.id == index) {
        ...
        $scope.editorFactory.promise.then(function () {
            ...
            $http.get(ACTIVITI.CONFIG.jsonUrl).success(function(data){
                $scope.jsonData=data;

                /*用于校验的jsonData
                 *属性面板点击下拉列表动态显示隐藏属性功能需要去掉隐藏的属性的校验，避免点击保存时校验隐藏的属性
                 */
                $scope.jsonDataforValid = _.cloneDeep(data);

                //保存json里的所有propertyPackages对象
                $scope.propertyPackages = {};
                data.propertyPackages.length && data.propertyPackages.forEach(function(item){
                  $scope.propertyPackages['oryx-' + item.properties[0].id] = item.properties[0];
                });
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    // 获取当前选中的节点的id
                    $scope.nodeId = shapes[0].resourceId;
                    ...
                    if (canvasSelected) {
                        ...
                        selectedItem.canvasSelected=canvasSelected;
                    }

                    //初始化属性分组信息(深拷贝)
                    var temp = JSON.parse(JSON.stringify(KISBPM.PROPERTY_GROUP));
                    var propertyGroup = typeof temp == 'string' ? JSON.parse(temp) : temp;


                    //目前只有人工任务和服务任务支持校验
                    var needValid = false;
                    if (stencil.id().indexOf("UserTask") !== -1 || stencil.id().indexOf("ServiceTask") !== -1) {
                      needValid = true;
                    }

                    //是否是人工任务
                    var isUserTask = false;
                    if(stencil.id().indexOf("UserTask") !== -1) {
                      isUserTask = true;
                    }
                    ...
                    for (var i = 0; i < properties.length; i++) {
                        ...
                        if (propertyConfig === undefined || propertyConfig === null) {
                            ...
                            //获取属性分组，没有则分到扩展属性
                            var group = null;
                            if(property._jsonProp.group != undefined) {
                              group =  property._jsonProp.group;
                            } else {
                              group = 0;
                            }

                            //获取校验属性(目前只有人工任务和服务任务支持校验)
                            var valid = property._jsonProp.valid && needValid ? property._jsonProp.valid : {};
                            var inputTip =  property._jsonProp.inputTip && needValid ? property._jsonProp.inputTip : '';
                            ...
                            var currentProperty = {
                                ...
                                'value': selectedShape.properties[key] || property._jsonProp.value,
                                'group': group,
                                'valid': valid,
                                'inputTip': inputTip,
                                'hideProperties': property._jsonProp.hideProperties,  //存放需要隐藏的属性
                                'show': true,  //是否显示该属性，默认全部显示，用于填写某些属性属性时动态显示隐藏其他属性
                                'selectOptions': property._jsonProp.selectOptions,  //下拉列表的选项
                                'toolTip': property._jsonProp.toolTip //表单信息提示
                            };

                            //隐藏到期的日期属性，该属性值由超时时间值拼接，不再是用户手动输入
                            //隐藏任务监听器
                            if (key.indexOf('duedatedefinition') !== -1 || key.indexOf('tasklisteners') !== -1) {
                              currentProperty.show = false;
                            }

                            //只显示人工任务的多实例属性
                            if (!isUserTask && key.indexOf('multiinstance') !== -1) {
                              currentProperty.show = false;
                            }

                            //统计每个分组的属性个数，不包括隐藏的属性
                            propertyGroup.some(function(item) {
                              if (item.id === group && currentProperty.show) {
                                item.number ++;
                                return true;
                              }
                            });
