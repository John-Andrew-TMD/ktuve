...
ORYX.Core.StencilSet.Stencil = {
	...
	construct: function(jsonStencil, namespace, source, stencilSet, propertyPackages, defaultPosition) {
		...
		var url = ACTIVITI.CONFIG.viewUrl + "view/" + jsonStencil.view;
...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
    ...
    construct: function(source, modelMetaData, editorId){
	  ...
	  //       });
	  	new Ajax.Request(ACTIVITI.CONFIG.jsonUrl, {
	          asynchronous: false,
	          method: 'get',
	          onSuccess: this._init.bind(this),
	          onFailure: this._cancelInit.bind(this)
    ...
    __handleStencilset: function(response){
        ...
        try {
            ...
            /*请求属性下拉列表数据*/
            Promise.all([
              getCallbackOptions('获取流程启动回调接口', 4),
              getCallbackOptions('获取流程运行时回调接口', 6),
              getCallbackOptions('获取流程结束回调接口', 5),
              getCallbackOptions('获取任务监听器', 1),
              getCallbackOptions('获取超时延期规则', 2),
            ])
              .then(function(options) {
                //设置回调接口下拉列表默认值和选项
                self._jsonObject.propertyPackages.forEach(function(item) {
                  if (item.name === 'global_start_callback_package') {
                    item = setCallbackOptions(options[0], item);
                  } else if (['global_run_callback_package', 'biz_node_run_callback_package'].includes(item.name)) {
                    item = setCallbackOptions(options[1], item);
                  } else if (item.name === 'global_end_callback_package') {
                    item = setCallbackOptions(options[2], item);
                  } else if (['prelistenerpackage', 'postlistenerpackage'].includes(item.name)) {
                    item = setCallbackOptions(options[3], item);
                  } else if (item.name === 'timeouthandlerpackage') {
                    item = setCallbackOptions(options[4], item);
                  }
                });
                jsonData = self._jsonObject;
              }).catch(function(err) {
              console.log(err);
            });
        }
	...
	_getPluginFacade: function() {
		...
		if(!(this._pluginFacade))
			...
			this._pluginFacade = {
				...
				getEditType:			this.getEditType.bind(this),
				setEditType:			this.setEditType.bind(this)
...
ORYX.Plugins.RenameShapes = Clazz.extend({
	...
	actOnDBLClick: function(evt, shape){
		...
		//若当前编辑类型为流程方案，则禁止修改名称
		if(this.facade.getEditType() == 'solution') {
			return;
		}
...
ORYX.Plugins.DragDropResize = ORYX.Plugins.AbstractPlugin.extend({
	...
	construct: function(facade) {
		...
		//查询当前编辑器类型
		this.editType 			= this.facade.getEditType();
	...
	handleMouseDown: function(event, uiObj) {
		...
		//若是流程方案配置，则禁止拖动
		// if(this.editType != 'solution') {
		// 	this.dragEnable = true;
		// }

		this.dragEnable = true; //改为可以拖动
...
//获取属性下拉列表信息
function getCallbackOptions(type, param) {
  return new Promise(function(resolve, reject){
    axios.get(KISBPM.URL.getCallbackOptions(param))
      .then(function(res) {resolve(res.data);})
      .catch(function(err) {reject(type + '失败:' + err);});
  });
}

//设置回调接口下拉列表默认值和选项
function setCallbackOptions(optionRe, item) {
  var options = [{
      'key': '',    //设置默认选项
      'value': '无'
   }];
  if (optionRe.code === 200 && optionRe.resultData && optionRe.resultData.length) {
    var re = optionRe.resultData;
    //设下拉列表选项
    re.forEach(function(obj) {
      var temp = {};
      temp.key = obj.beanId;
      temp.value = obj.beanName;
      options.push(temp);
    });
    item.properties[0].selectOptions = options;
  }
  return item;
}
